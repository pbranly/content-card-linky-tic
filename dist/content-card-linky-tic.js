...
if (oldHass) {
return (
oldHass.states[element.config.tempoEntityBBRHCJB] !==
element.hass.states[element.config.tempoEntityBBRHCJB] ||
oldHass.states[element.config.tempoEntityBBRHPJB] !==
element.hass.states[element.config.tempoEntityBBRHPJB] ||
oldHass.states[element.config.tempoEntityHCJB] !==
element.hass.states[element.config.tempoEntityHCJB] ||
oldHass.states[element.config.tempoEntityHPJB] !==
element.hass.states[element.config.tempoEntityHPJB] ||
oldHass.states[element.config.tempoEntityHCJR] !==
element.hass.states[element.config.tempoEntityHCJR] ||
oldHass.states[element.config.tempoEntityHPJR] !==
element.hass.states[element.config.tempoEntityHPJR]
);
}
...
// Fonction pour calculer la consommation totale à partir des sensors tempo
...
// Récupération des valeurs des différents sensors tempo
const tempoEntityBBRHCJB = this.hass.states[this.config.tempoEntityBBRHCJB];
const tempoEntityBBRHPJB = this.hass.states[this.config.tempoEntityBBRHPJB];
const tempoEntityHCJB = this.hass.states[this.config.tempoEntityHCJB];
const tempoEntityHPJB = this.hass.states[this.config.tempoEntityHPJB];
const tempoEntityHCJR = this.hass.states[this.config.tempoEntityHCJR];
const tempoEntityHPJR = this.hass.states[this.config.tempoEntityHPJR];

// Addition de toutes les consommations disponibles
if (tempoEntityBBRHCJB && !isNaN(parseFloat(tempoEntityBBRHCJB.state))) {
  totalConsumption += parseFloat(tempoEntityBBRHCJB.state);
}
if (tempoEntityBBRHPJB && !isNaN(parseFloat(tempoEntityBBRHPJB.state))) {
  totalConsumption += parseFloat(tempoEntityBBRHPJB.state);
}
if (tempoEntityHCJB && !isNaN(parseFloat(tempoEntityHCJB.state))) {
  totalConsumption += parseFloat(tempoEntityHCJB.state);
}
if (tempoEntityHPJB && !isNaN(parseFloat(tempoEntityHPJB.state))) {
  totalConsumption += parseFloat(tempoEntityHPJB.state);
}
if (tempoEntityHCJR && !isNaN(parseFloat(tempoEntityHCJR.state))) {
  totalConsumption += parseFloat(tempoEntityHCJR.state);
}
if (tempoEntityHPJR && !isNaN(parseFloat(tempoEntityHPJR.state))) {
  totalConsumption += parseFloat(tempoEntityHPJR.state);
}
...
// Fonction pour créer un objet attributes simulé à partir des sensors tempo
createAttributesFromTempoSensors() {
const tempoEntityBBRHCJB = this.hass.states[this.config.tempoEntityBBRHCJB];
const tempoEntityBBRHPJB = this.hass.states[this.config.tempoEntityBBRHPJB];
const tempoEntityHCJB = this.hass.states[this.config.tempoEntityHCJB];
const tempoEntityHPJB = this.hass.states[this.config.tempoEntityHPJB];
...
```
// Utiliser les attributs du premier sensor disponible comme base
let baseAttributes = {};
if (tempoEntityBBRHCJB) {
  baseAttributes = { ...tempoEntityBBRHCJB.attributes };
} else if (tempoEntityHCJB) {
  baseAttributes = { ...tempoEntityHCJB.attributes };
} else if (tempoEntityHPJB) {
  baseAttributes = { ...tempoEntityHPJB.attributes };
}

// Calculs spécifiques pour HC/HP
const yesterday_HC = (tempoEntityBBRHCJB ? parseFloat(tempoEntityBBRHCJB.state) || 0 : 0) + 
                     (tempoEntityHCJB ? parseFloat(tempoEntityHCJB.state) || 0 : 0) + 
                     (tempoEntityHCJR ? parseFloat(tempoEntityHCJR.state) || 0 : 0);

const yesterday_HP = (tempoEntityBBRHPJB ? parseFloat(tempoEntityBBRHPJB.state) || 0 : 0) + 
                     (tempoEntityHPJB ? parseFloat(tempoEntityHPJB.state) || 0 : 0) + 
                     (tempoEntityHPJR ? parseFloat(tempoEntityHPJR.state) || 0 : 0);

return {
  ...baseAttributes,
  typeCompteur: "consommation",
  yesterday_HC: yesterday_HC,
  yesterday_HP: yesterday_HP,
  unit_of_measurement: baseAttributes.unit_of_measurement || "kWh",
  // Calcul du pourcentage HP/HC
  peak_offpeak_percent: yesterday_HP + yesterday_HC > 0 ? (yesterday_HP / (yesterday_HP + yesterday_HC)) * 100 : 0,
  // Vous pouvez ajouter d'autres calculs selon vos besoins
  daily_cost: baseAttributes.daily_cost || 0,
  yearly_evolution: baseAttributes.yearly_evolution || 0,
  monthly_evolution: baseAttributes.monthly_evolution || 0,
  current_month_evolution: baseAttributes.current_month_evolution || 0,
  current_week_evolution: baseAttributes.current_week_evolution || 0,
  yesterday_evolution: baseAttributes.yesterday_evolution || 0,
  daily: baseAttributes.daily || [],
  dailyweek: baseAttributes.dailyweek || [],
  dailyweek_cost: baseAttributes.dailyweek_cost || [],
  dailyweek_costHC: baseAttributes.dailyweek_costHC || [],
  dailyweek_costHP: baseAttributes.dailyweek_costHP || [],
  dailyweek_HC: baseAttributes.dailyweek_HC || [],
  dailyweek_HP: baseAttributes.dailyweek_HP || [],
  dailyweek_MP: baseAttributes.dailyweek_MP || [],
  dailyweek_MP_over: baseAttributes.dailyweek_MP_over || [],
  dailyweek_MP_time: baseAttributes.dailyweek_MP_time || [],
  dailyweek_Tempo: baseAttributes.dailyweek_Tempo || [],
  errorLastCall: baseAttributes.errorLastCall || "",
  versionUpdateAvailable: baseAttributes.versionUpdateAvailable || false,
  versionGit: baseAttributes.versionGit || "",
  serviceEnedis: baseAttributes.serviceEnedis || "myElectricalData"
};
```
...
```
// Vérifier qu'au moins un sensor tempo est configuré
if (!this.config.tempoEntityBBRHCJB && !this.config.tempoEntityBBRHPJB && 
    !this.config.tempoEntityHCJB && !this.config.tempoEntityHPJB && 
    !this.config.tempoEntityHCJR && !this.config.tempoEntityHPJR) {
  return html`
    <ha-card>
      <div class="card">
        <div id="states">
          <div class="name">
            ...
            <span style="margin-right:2em">Linky : aucun sensor tempo configuré</span>
          </div>
        </div>
      </div>
    </ha-card> 
...
// Créer un objet stateObj simulé
const totalConsumption = this.calculateTotalConsumption();
const attributes = this.createAttributesFromTempoSensors();
...
if (stateObj) {
    if (( modeCompteur === "consommation" ) || ( !modeCompteur )){
      return html`
        <ha-card id="card">
          ${this.addEventListener('click', event => { this._showDetails('tempo_sensors'); })}
...
// Pour les sensors tempo, on peut afficher les détails du premier sensor disponible
let entityToShow = this.config.tempoEntityBBRHCJB || this.config.tempoEntityHCJB || this.config.tempoEntityHPJB;
...
return html` <div class="main-info"> ${this.renderIcon(attributes, config)} <div class="hp-hc-block"> <span class="conso-hc">${this.toFloat(attributes.yesterday_HC)}</span><span class="conso-unit-hc"> ${attributes.unit_of_measurement} <span class="more-unit">(en HC)</span></span><br /> <span class="conso-hp">${this.toFloat(attributes.yesterday_HP)}</span><span class="conso-unit-hp"> ${attributes.unit_of_measurement} <span class="more-unit">(en HP)</span></span> </div> ${this.renderPrice(attributes, config)} </div>`
}
else{
return html` <div class="main-info"> ${this.renderIcon(attributes, config)} <div class="cout-block"> <span class="cout">${this.toFloat(stateObj.state)}</span> <span class="cout-unit">${attributes.unit_of_measurement}</span> </div> ${this.renderPrice(attributes, config)} </div>`
}
}
}
renderIcon(attributes, config) {
if ( this.config.showIcon ){
return html ` <div class="icon-block"> <span class="linky-icon bigger" style="background: none, url('/local/community/content-card-linky-tic/icons/linky.svg') no-repeat; background-size: contain;"></span> </div>`
}
else{
return html `} } renderPrice(attributes, config) { if ( this.config.showPrice ){ return html ` <div class="cout-block"> <span class="cout" title="Coût journalier">${this.toFloat(attributes.daily_cost, 2)}</span><span class="cout-unit"> €</span> </div>` } else{ return html`
}
}
...
return html `} else{ if ( attributes.serviceEnedis !== "myElectricalData" ){ return html ` <div class="information-msg" style="color: red"> <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon> Merci de migrer sur myElectricalData.<br> EnedisGateway sera desactivé courant 2023. </div> ` } } } renderVersion(versionUpdateAvailable, versionGit) { if ( versionUpdateAvailable === true ){ return html ` <div class="information-msg" style="color: red"> <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon> Nouvelle version disponible ${versionGit} </div> ` } else{ return html`
}
}
...
return html
`<div class="week-history"> ${this.renderTitreLigne(config)} ${daily.slice(0, nbJours).reverse().map((day, index) => this.renderDay(day, nbJours-index, unit_of_measurement, dailyweek, dailyweek_cost, dailyweek_costHC, dailyweek_costHP,  dailyweek_HC, dailyweek_HP, dailyweek_MP, dailyweek_MP_over, dailyweek_MP_time, dailyweek_Tempo, config))} </div>`
}
}
}
...
return html
`<div class="day"> ${this.renderDailyWeek(dailyweek, dailyweek_Tempo, dayNumber, config)} ${this.renderDailyValue(day, dayNumber, unit_of_measurement, config)} ${this.renderDayPrice(dailyweek_cost, dayNumber, config)} ${this.renderDayPriceHCHP(dailyweek_costHC, dayNumber, config)} ${this.renderDayPriceHCHP(dailyweek_costHP, dayNumber, config)} ${this.renderDayHCHP(dailyweek_HC, dayNumber, unit_of_measurement, config)} ${this.renderDayHCHP(dailyweek_HP, dayNumber, unit_of_measurement, config)} ${this.renderDayMaxPower(dailyweek_MP, dayNumber, dailyweek_MP_over, dailyweek_MP_time, config)} </div>`
}
...
return html
`<div class="day"> ${this.renderDailyWeekTitre(true, "")} ${this.renderDailyWeekTitre(true, "Conso")} ${this.renderDailyWeekTitre(this.config.showDayPrice, "Prix")} ${this.renderDailyWeekTitre(this.config.showDayPriceHCHP, "Prix HC")} ${this.renderDailyWeekTitre(this.config.showDayPriceHCHP, "Prix HP")} ${this.renderDailyWeekTitre(this.config.showDayHCHP, "HC")} ${this.renderDailyWeekTitre(this.config.showDayHCHP, "HP")} ${this.renderDailyWeekTitre(this.config.showDayMaxPower, "MP")} ${this.renderDailyWeekTitre(this.config.showDayMaxPowerTime, "MPtime")} </div>`
}
}
r_enderTitreLigne(config) {
if (this.config.showTitleLign === true) {
return html
`<div class="day"> <br><span class="cons-val">Conso.</span> ${this.config.showDayPrice  ? html`
<br><span class="cons-val">Prix</span>`: html `` } ${this.config.showDayPriceHCHP ? html`
<br><span class="cons-val">Prix HC</span>`: html `` } ${this.config.showDayPriceHCHP  ? html`
<br><span class="cons-val">Prix HP</span>`: html `` } ${this.config.showDayHCHP  ? html`
<br><span class="cons-val">HC</span>`: html `` } ${this.config.showDayHCHP  ? html`
<br><span class="cons-val">HP</span>`: html `` } ${this.config.showDayMaxPower  ? html`
<br><span class="cons-val">MP</span>`: html `` } ${this.config.showDayMaxPower  ? html`
<br><span class="cons-val">MPtime</span>`: html `` } </div>`;
}
}
...
return html
`<br><span class="cons-val">${this.toFloat(day)}  ${this.config.showInTableUnit  ? html`
${unit_of_measurement}`: html `` }</span>`;
}
}
...
return html
`<br><span class="cons-val">${this.toFloat(valeur, 2)}  ${this.config.showInTableUnit  ? html`
${unit_of_measurement}`: html `` }</span>`;
}
}
}
...
if ( over === “true”) {
return html
`<br><span class="cons-val" style="color:red">${this.toFloat(valeur, 2)}</span> <br><span class="cons-val" style="color:red">${new Date(MPtime.toString().split(",")[dayNumber-1]).toLocaleTimeString('fr-FR', { hour: "2-digit", minute: "2-digit" }) }</span>`;
}
else {
return html
`<br><span class="cons-val">${this.toFloat(valeur, 2)}</span> <br><span class="cons-val">${new Date(MPtime.toString().split(",")[dayNumber-1]).toLocaleTimeString('fr-FR', { hour: "2-digit", minute: "2-digit" }) }</span>`;
}
}
}
}
...
if (attributes.serviceEnedis === undefined ){
return html `; } if ( attributes.serviceEnedis !== "myElectricalData" ){ return html `EcoWatt : uniquement disponible avec myElectricData`; } if (this.config.showTempo === false ){ return html `;
}
...
setConfig(config) {
if (!config.tempoEntityBBRHCJB && !config.tempoEntityBBRHPJB &&
!config.tempoEntityHCJB && !config.tempoEntityHPJB &&
!config.tempoEntityHCJR && !config.tempoEntityHPJR) {
throw new Error(‘You need to define at least one tempo entity’);
}
...
const defaultConfig = {
  ...
  nbJoursAffichage: "7",
  kWhPrice: undefined,
  // Nouveaux paramètres pour les sensors tempo
  tempoEntityBBRHCJB: undefined,
  tempoEntityBBRHPJB: undefined,
  tempoEntityHCJB: undefined,
  tempoEntityHPJB: undefined,
  tempoEntityHCJR: undefined,
  tempoEntityHPJR: undefined,
}
