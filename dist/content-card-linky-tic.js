...
window.customCards.push({
    ...
    preview: true,
    documentationURL: "[https://github.com/pbranly/content-card-linky-tic](https://github.com/pbranly/content-card-linky-tic)",
});
...
class ContentCardLinkyTic extends LitElement {
    ...
    renderError(errorMsg, config) {
        if (this.config.showError === true && errorMsg != "") {
            return html`
                <div class="error-msg" style="color: red">
                    <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon> ${errorMsg}
                </div>`;
        }
        return html``;
    }
    ...
    renderInformation(attributes, config) {
        if (attributes.serviceEnedis !== "myElectricalData" && attributes.serviceEnedis !== undefined) {
            return html`
                <div class="information-msg" style="color: red">
                    <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon> Merci de migrer sur myElectricalData.<br> EnedisGateway sera desactivé courant 2023.
                </div>`;
        }
        return html``;
    }
    ...
    renderVersion(versionUpdateAvailable, versionGit) {
        if (versionUpdateAvailable === true) {
            return html`
                <div class="information-msg" style="color: red">
                    <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon> Nouvelle version disponible ${versionGit}
                </div>`;
        }
        return html``;
    }
            ...
            if (dailyweek !== undefined) {
                ...
                return html`
                    <div class="week-history">
                        ...
                        ${daily.slice(0, nbJours).reverse().map((day, index) => this.renderDay(day, nbJours - index, unit_of_measurement, dailyweek, dailyweek_cost, dailyweek_costHC, dailyweek_costHP, dailyweek_HC, dailyweek_HP, dailyweek_MP, dailyweek_MP_over, dailyweek_MP_time, dailyweek_Tempo, config))}
                    </div>`;
            }
        }
        return html``;
        ...
        if (this.config.showTempoIndex === true) {
            return html`
                <div class="tempo-index">
                    <div class="tempo-index-title">Index Tempo</div>
                    <table class="tempo-index-table">
                        <tr>
                            <td class="tempo-blue">Jours Bleus</td>
                            <td>HC: ${indexBBRHCJB ? this.toFloat(indexBBRHCJB.state) : 'N/A'} kWh</td>
                            <td>HP: ${indexBBRHPJB ? this.toFloat(indexBBRHPJB.state) : 'N/A'} kWh</td>
                        </tr>
                        <tr>
                            <td class="tempo-white">Jours Blancs</td>
                            <td>HC: ${indexBBRHCJW ? this.toFloat(indexBBRHCJW.state) : 'N/A'} kWh</td>
                            <td>HP: ${indexBBRHPJW ? this.toFloat(indexBBRHPJW.state) : 'N/A'} kWh</td>
                        </tr>
                        <tr>
                            <td class="tempo-red">Jours Rouges</td>
                            <td>HC: ${indexBBRHCJR ? this.toFloat(indexBBRHCJR.state) : 'N/A'} kWh</td>
                            <td>HP: ${indexBBRHPJR ? this.toFloat(indexBBRHPJR.state) : 'N/A'} kWh</td>
                        </tr>
                    </table>
                </div>`;
        }
    ...
    renderDailyValue(day, dayNumber, unit_of_measurement, config) {
        if (day === -1) {
            ...
            return html`
                <br><span class="cons-val">${this.toFloat(day)} ${this.config.showInTableUnit ? html`${unit_of_measurement}` : html``}</span>`;
        }
    }
    ...
    renderDayHCHP(value, dayNumber, unit_of_measurement, config) {
        if (config.showDayHCHP) {
            ...
            if (valeur === "-1") {
                ...
                return html`
                    <br><span class="cons-val">${this.toFloat(valeur, 2)} ${this.config.showInTableUnit ? html`${unit_of_measurement}` : html``}</span>`;
            }
        }
        return html``;
    }
    ...
    renderEcoWatt(attributes, config) {
        if (attributes.serviceEnedis === undefined || attributes.serviceEnedis !== "myElectricalData") {
    ...
    renderTempo(attributes, config) {
        if (attributes.serviceEnedis === undefined || attributes.serviceEnedis !== "myElectricalData" || this.config.showTempo === false) {
            return html``;
        }
        ...
        if (!tempoJ0 || !tempoJ1 || !tempoInfo) {
            return html`Tempo: capteur(s) J0 et/ou J1 et/ou info indisponible(s) ou incorrect(s)`;
        }
