...
try {
await import(”/local/community/content-card-linky-tic/content-card-linky-tic-editor.js”);
} catch (error) {
console.warn(“Could not load editor:”, error);
}
...
// Recuperation des donnees depuis les differents capteurs linky_tempo
...
// Nouvelle methode pour recuperer les donnees des capteurs linky_tempo
getLinkyTempoData() {
const sensors = {
...
// Calcul basé sur les index actuels
...
// Simulation d’un historique base sur les donnees actuelles
// Dans un vrai cas, il faudrait utiliser l’historique de Home Assistant
const history = [];
...
for (let i = 6; i >= 0; i--) {
  ...
  // Simulation de donnees historiques (a adapter selon vos besoins)
  const dayData = {
    date: date,
...
// Logique pour determiner la couleur tempo d’une date
// A adapter selon votre source de donnees tempo
const colors = [“BLUE”, “WHITE”, “RED”];
return colors[Math.floor(Math.random() * colors.length)];
}

calculateDayCost(dayData) {
// Calcul du cout base sur les 6 tarifs tempo
let tarifHP, tarifHC;
...
```
// Pour l'instant, on simule les donnees tempo
...
```
// Verification des capteurs linky_tempo requis
...
const defaultConfig = {
  ...
  titleName: "LINKY",
  nbJoursAffichage: "7",
  // Nouveaux tarifs tempo (6 tarifs differents)
